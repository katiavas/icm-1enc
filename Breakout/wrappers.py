...
class Step(gym.Wrapper):
    def __init__(self, env, n_repeat=4, fire = True):
...
class StackFrames(gym.ObservationWrapper):
    def __init__(self, env, n_repeat):
        ...
        self.observation_space = gym.spaces.Box(
            env.observation_space.low.repeat(n_repeat, axis=0),
            env.observation_space.high.repeat(n_repeat, axis=0),
            dtype=np.float32)
        # Set our stack which will be a deque of maxlen repeat
        self.stack = collections.deque(maxlen=n_repeat)
...
def make_atari(env_name, shape=(42, 42, 1), n_repeat=4):
    env = gym.make(env_name)
    # env.seed(111)
    env = Step(env, n_repeat)
    env = PreprocessFrame(shape, env)
    env = StackFrames(env, n_repeat)
    return env
